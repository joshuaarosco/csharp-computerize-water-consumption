...
Public Class frm_dashboard
    ...
    Private Sub txt_search_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txt_search.KeyDown
        'search()
    End Sub
    ...
    Sub search()
        lbl_result.Text = txt_search.Text.ToString()

        For Each row As DataGridViewRow In datagrid_transaction.SelectedRows
            datagrid_transaction.Rows.Remove(row)
        Next
        ...
        Try
            MysqlConn.Open()
            Dim Query As String
            Query = "select * from computerized_water_consumption_db.user where id <> '1' and fname like '%" & txt_search.Text.ToString() & "%' or lname like '%" & txt_search.Text.ToString() & "%' or contact_number like '%" & txt_search.Text.ToString() & "%' or address like '%" & txt_search.Text.ToString() & "%'  order by created_at desc"
            Dim Adapter As New MySqlDataAdapter(Query, MysqlConn)
            Dim Dataset As New DataSet()
